		...
		fn hrmp_init_open_channel(
			...
			let notification_bytes = {
				...
				Xcm::HrmpNewChannelOpenRequest {
					...
					max_capacity: max_places,
					max_message_size,
				}.encode()
			};
			if let Err(dmp::QueueDownwardMessageError::ExceedsCriticalMessageSize) =
			...
			{
				// this should never happen unless the critical message size is configured to an
